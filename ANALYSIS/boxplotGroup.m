...
x = cell(1,nMembers);
existingTextObjs = findobj(h.axis,'Type','Text');
boxch = gobjects(1,nMembers); 
for i = 1:nMembers
    x{i} = i : xInterval : maxX;
    xExpanded = repelem(i : xInterval : maxX, height(p.Results.x{i}),1);
    boxch(i) = boxchart(xExpanded(:), p.Results.x{i}(:), unmatchNameVal{:});
end
